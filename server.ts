...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/met-art/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/met-art/browser'),
        'index.html'
      )
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: `MetArt.tube Research ${group?.PageTitle}`,
